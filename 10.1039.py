...
def download_supporting_information(doi, SiPDFName):
    ...
    # 查找新的支持信息链接
    for link in soup.find_all('a', class_='list__item-link'):
        href = link.get('href')
    ...
    for supporting_info_link in supporting_info_links:
        if supporting_info_link.startswith('/'):
            supporting_info_link = f"https://www.rsc.org{supporting_info_link}"
...
if __name__ == "__main__":
    dir = '...'
    file_name = '10.1039.csv'
    file_path = os.path.join(dir, file_name)

    # 读取 CSV 文件
    raw_df = pd.read_csv(file_path)

    # 筛选出 processed 列为 False 的项
    df = raw_df[raw_df['processed'] == False]

    # doi = '10.1039/c2nr33611a'
    # SiPDFName = 'si_10.1039_c2nr33611a'
    ...
    for index, row in df.iterrows():
        ...
        output_file = os.path.join(dir, '10.1039.csv')
