...
def download_supporting_information(doi, SiPDFName):
    ...
    # 查找新网站的支持信息链接
    for link in soup.find_all('a', class_='print-link'):
        href = link.get('href')
    ...
    for supporting_info_link in supporting_info_links:
        if supporting_info_link.startswith('/'):
            supporting_info_link = f"https://static-content.springer.com{supporting_info_link}"
...
if __name__ == "__main__":
    ...
    # 筛选出 DOI 以 "10.1038" 开头的行
    df = raw_df[raw_df['DOI'].str.startswith('10.1038')]
    df.loc[:, 'processed'] = False

    # doi = '10.1038/nnano.2013.181'
    # SiPDFName = 'si_10.1038_nnano.2013.181'
    ...
    for index, row in df.iterrows():
        ...
        output_file = os.path.join(dir, '10.1038.csv')
