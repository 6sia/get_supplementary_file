...
def download_supporting_information(doi, SiPDFName):
    ...
    # 查找特定的 td 标签
    for td in soup.find_all('td', headers='article-filename'):
        link = td.find('a')
        if link and link.get('href'):
            href = link.get('href')
            if href.endswith(supported_formats):
    ...
    for supporting_info_link in supporting_info_links:
        if supporting_info_link.startswith('/'):
            supporting_info_link = f"https://onlinelibrary.wiley.com{supporting_info_link}"
...
if __name__ == "__main__":
    ...
    # 筛选出 DOI 以 "10.1002" 开头的行
    df = raw_df[raw_df['DOI'].str.startswith('10.1002')]
    df.loc[:, 'processed'] = False

    # doi = '10.1002/adbi.201800221'
    # SiPDFName = 'si_10.1002_adbi.201800221'
    ...
    for index, row in df.iterrows():
        ...
        output_file = os.path.join(dir, '10.1002.csv')
